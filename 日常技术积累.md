...
github上面打开目录后，在文件的中间点击(commit时添加的-m "信息"的)描述信息，红色表示上次的的内容，绿色的表示最新改动的内容，形成对照。
    
如果通过git客户端命令无法上传文件，则直接进入github个人仓库的对应文件里面，里面有相关上传文件选项,但是下一次记得push之前先pull，保持和远程仓库一致，防止造成冲突。
...
依次执行上面四条命令；再次push;不行就多试几次造成该问题的原因很有可能是网络不稳定，连接超时造成的。
 ...
 工作区：分为已控制文件 和新文件或有变动的文件(新变文件git status显示红色)
...
git reflog 可以查看到之前的版本号，再用git reset --hard 版本id 回到之前的版本即可。
 --在远程仓库，将本地库的文件filename经过 git push origin master 之后就推送到了远程仓库。注意在push之前，为了防止远程仓库被别人改动过，导致自己本地库和远程仓库不一致，首先要执行 git pull origin master 拉取远程仓库内容，将本地库和远程仓库保持一致，再push；如果遇到这种情况：推送的一个文件内容是错误的，你想重新推送一个正确的版本覆盖错误的版本，则在本地库将此版回退到正确的版本，然后直接强制提交：git push -f origin master。         
