...
kubernetes：自动运维容器程序的集群系统；处理诸如：1-一个容器故障停机了，怎么样让另外一个容器立刻启动去替补停机的容器；2-当并发访问量变大的时候，怎么样做到横向扩展容器数量；这些容器管理的问题统称为 容器编排 问题。
    1- kubernetes的本质上就是一个集群系统，用户可以在集群中部署各种服务，所谓的部署服务，其实就是在kubernetes集群中运行一个个的容器，并将指定的程序跑在容器中。

	2-kubernetes的最小管理单元是pod而不是容器，所以只能将容器放在Pod中，而kubernetes一般也不会直接管理Pod，而是通过Pod控制器来管理Pod的。
...
学习kubernetes的核心，就是学习如何对集群上的Pod、Pod控制器、Service、存储等各种资源进行操作。
        ...
        namespace 简写 ns--由于pod之间可以相互访问，如果不想pod之间相互访问，则将pod加入到不同的namespace。
        pod  包含一个或多个docker容器。
     ...
     上面关系：pod里面有很多资源，这些资源都有label，deployment或service要想找到访问这些指定的资源，则通过中间件label selector来找到pod里面对应的资源(不一定是pod里面资源才有label，pod、node、service都有label)
```
